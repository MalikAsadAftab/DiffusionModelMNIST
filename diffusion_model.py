...
class DiffusionModel(nn.Module):
    """Class for training and generating images using a diffusion model."""
    ...
    def __init__(self, device=None, dataset_name=None, checkpoint_name=None):
        ...
        # Initialize dataset name and checkpoint if provided
    ...
    def sample_ddpm(self, n_samples, context=None, timesteps=None, 
        ...
        """Returns the final denoised sample x0, intermediate samples, and time steps."""
    ...
    def get_transforms(self, dataset_name):
        ...
        if dataset_name in {"mnist", "fashion_mnist", "cifar10"}:
            transform = transforms.Compose([
                transforms.ToTensor(),
                lambda x: 2 * (x - 0.5)  # Normalize to range [-1, 1]
            ])
    ...
    def get_x_unpert(self, x_pert, t, pred_noise, ab_t):
        """Removes predicted noise from perturbed image."""
    ...
    def save_generated_samples_into_folder(self, n_samples, context, folder_path, **kwargs):
        """Saves DDPM generated samples into a specified directory."""
    ...
    def save_dataset_test_images(self, n_samples):
        """Saves dataset test images with specified number."""
    ...
    def generate(self, n_samples, n_images_per_row, timesteps, beta1, beta2):
        """Generates x0 and intermediate samples via DDPM, and saves as jpeg and gif files."""
...
Importing necessary libraries for handling datasets, creating and saving images, and model operations.

DiffusionModel Class:

__init__: Initializes the DiffusionModel with specified device, dataset name, and checkpoint name.

train: Trains the model using the specified parameters, including batch size, number of epochs, learning rate, timesteps, and noise schedule parameters.

sample_ddpm: Samples the diffusion model to generate images, returning the final denoised sample and intermediate samples.

perturb_input: Perturbs the input with noise according to the diffusion process.

instantiate_dataset: Instantiates the dataset based on the specified name.

get_transforms: Returns the transformation functions for the specified dataset.

get_x_unpert: Removes predicted noise from the perturbed image.

initialize_nn_model: Initializes the neural network model based on the dataset name and checkpoint.

save_checkpoint: Saves the model checkpoint.

create_dirs: Creates necessary directories for saving checkpoints and images.

initialize_optimizer: Initializes the optimizer, loading from a checkpoint if provided.

initialize_scheduler: Initializes the learning rate scheduler, loading from a checkpoint if provided.

get_start_epoch: Returns the starting epoch for training.

save_tensor_images: Saves tensor images as a single image file.

get_ddpm_noise_schedule: Returns the DDPM noise schedule parameters.

get_ddpm_params_from_checkpoint: Returns the DDPM parameters from a checkpoint.

denoise_add_noise: Removes predicted noise and adds Gaussian noise according to the diffusion process.

initialize_dataset_name: Initializes the dataset name based on the checkpoint if available.

initialize_dataloader: Initializes the data loader, using the batch size from the checkpoint if available.

get_masked_context: Randomly masks out context.

save_generated_samples_into_folder: Saves generated samples into a specified directory.

save_dataset_test_images: Saves test images from the dataset.

initialize_device: Initializes the device based on availability (CPU, GPU, or MPS).

get_custom_context: Returns custom context in one-hot encoded form.

generate: Generates images and saves them as JPEG and GIF files.
